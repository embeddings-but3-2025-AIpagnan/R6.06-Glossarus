...
def __serialise__(path=None, lexicalRes=None):
    ...
    # read config and resolve paths relative to the config file directory
    ...
    if not os.path.isabs(serialized_path):
        serialized_path = os.path.join(config_dir, serialized_path)
...
def load(path=None):
    ...
    if path is None:
        ...
        serialized_path = config.get('File_paths', 'resyf_path_xml_file_serialized')
        # resolve relative paths with the directory that contains config.cfg
        config_dir = os.path.dirname(config_path)
        if not os.path.isabs(xml_path):
            xml_path = os.path.join(config_dir, xml_path)
        if not os.path.isabs(serialized_path):
            serialized_path = os.path.join(config_dir, serialized_path)
        try:
...
def get_all_lexicalEntry(lexicalRes, path=None):
    ...
    if not os.path.isabs(output_path):
        output_path = os.path.join(os.path.dirname(config_path), output_path)
